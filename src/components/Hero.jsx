    ...
    <div className="max-w-7xl mx-auto px-4 pt-32 pb-12">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <motion.div
          ...
          className="lg:col-span-4 lg:row-span-2 bento-card bento-featured flex flex-col justify-between min-h-[400px]"
        >
          <div className="absolute inset-0 z-0 bg-gradient-to-br from-zinc-900/50 to-bg border border-white/5 rounded-2xl overflow-hidden">
             <div className="absolute inset-0 flex items-center justify-center opacity-20 pointer-events-none select-none">
                <span className="font-display font-black text-7xl md:text-9xl uppercase italic tracking-tighter text-white">RALLY DIRT</span>
             </div>
             <div className="absolute inset-0 bg-gradient-to-r from-bg via-bg/60 to-transparent"></div>
          </div>
          <div className="relative z-10">
            <div className="pill inline-block mb-6 uppercase tracking-widest text-[10px]">Featured Engine</div>
            <h1 className="font-display text-4xl md:text-6xl font-extrabold tracking-tighter mb-4 leading-none uppercase italic">
              RALLY RACER <br />
              <span className="text-brand">DIRT DRIFT</span>
            </h1>
            <p className="text-text-dim text-lg max-w-sm leading-snug">
              Master the art of off-road drifting. Optimized for high-speed performance on the MAXED architecture.
            </p>
          </div>
          
          <div className="flex gap-4 mt-8 relative z-10">
            <Link to="/lesson/1" className="cta-button">Launch</Link>
            <Link to="/lessons" className="bg-white/5 text-white font-bold px-6 py-3 rounded-[12px] text-sm hover:bg-white/10 transition-all">
              Browse More
            </Link>
          </div>
        </motion.div>
